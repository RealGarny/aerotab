  ...
  const Cols = useCallback(() => {
    ...
    return (
      <div className={'relative'}>
        <div className='flex'>
          <Cell />
          <EnumeratedRow onCellClick={onCellClick} className={'flex'} count={table.cols} />
        </div>
      </div>
    );
  }, [table.cols]);
...
const EnumeratedRow: FC<
  { onCellClick?: (cell: TUICellInstance) => void; count: number } & ComponentProps<'div'>
> = ({ count, onCellClick, ...otherProps }) => {
  return (
    <div {...otherProps}>
...
const TableCell: FC<{ data: TCell }> = ({ data }) => {
  ...
  return (
    <Cell>
      <input onChange={handleChange} className={'ring-0 w-full h-full'} defaultValue={data.data} />
    </Cell>
  );
};
...
const Cell: FC<{
  onClick?: (i: TUICellInstance) => void;
  children?: ReactNode;
  isActive?: boolean;
}> = ({ children, isActive = false, onClick, ...props }) => {
  ...
  return (
    <div
      ...
      {...props}
      className={`w-16 h-10 ${!localIsActive ? 'border-gray-200 bg-white' : 'bg-blue-200 border-blue-300'} border  flex items-center justify-center`}
    >
      {children}
    </div>
  );
